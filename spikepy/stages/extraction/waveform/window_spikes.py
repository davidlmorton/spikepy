...
def determine_excluded_spikes(signal, spike_index_list, window_size, 
    ...
    excluded_spike_index_list  = []
    truncated_spike_index_list = []
    for i in xrange(len(spike_index_list)):
        ...
        if bi > 0 and ei < len(signal)-1:
            ...
            if i > 0 and bi < spike_index_list[i-1] + post_i:
                excluded_spike_index_list.append(si)
            # test for end overlapping with beginning of next spike
            elif (i < len(spike_index_list)-1 and 
                    ei > spike_index_list[i+1] - pre_i):
                excluded_spike_index_list.append(si)
            ...
            truncated_spike_index_list.append(si)

    return (good_spike_index_list, truncated_spike_index_list, 
                excluded_spike_index_list)
